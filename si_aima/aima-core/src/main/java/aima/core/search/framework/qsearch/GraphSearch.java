 ...
 * implementations for the primitive operations including node filtering
 * mechanisms to avoid that nodes of already explored states are selected for
 * expansion. In contrast to the pseudocode above, here, nodes resulting from
 * node expansion are added to the frontier even if nodes for the same states
 * already exist there. This makes it possible to use the implementation also in
 * combination with priority queue frontiers. This implementation avoids linear
 * costs for frontier node removal (compared to
 * {@link GraphSearchReducedFrontier}) and gets by without node comparator
 * knowledge.
 *
...
//	private Set<S> explored = new HashSet<>();
	...
	/**
	 * Inserts the node at the tail of the frontier if the corresponding state was
	 * not yet explored.
	 */
	@Override
...
//				System.out.println(" - Node reinserted in frontier - ");
	...
	/**
	 * Removes the node at the head of the frontier, adds the corresponding state to
	 * the explored set, and returns the node. Leading nodes of already explored
	 * states are dropped. So the resulting node state will always be unexplored
	 * yet.
	 * 
	...
	/**
	 * Helper method which removes nodes of already explored states from the head of
	 * the frontier.
	 */
